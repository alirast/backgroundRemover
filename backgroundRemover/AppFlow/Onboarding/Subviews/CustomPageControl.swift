...
class CustomPageControl: UIControl {
    ...
    private var currentPageIndicatorTintColor = UIColor.blueColor
    private var pageIndicatorTintColor = UIColor.white.withAlphaComponent(0.3)
    ...
    @objc private func handleTap(sender: UITapGestureRecognizer) {
        let location = sender.location(in: backgroundForCircle)
        
        for (index, subview) in backgroundForCircle.subviews.enumerated() {
    ...
    func nextPage() {
        ...
        UIView.animate(withDuration: 0.3, delay: 0, options: [.curveEaseInOut]) {
            self.backgroundForCircle.subviews[oldIndex].frame = CGRect(
                x: self.backgroundForCircle.subviews[oldIndex].frame.origin.x - self.correctionForPoints,
                y: 0,
                width: self.sizeForCircle,
                height: self.sizeForCircle)
            ...
            self.backgroundForCircle.subviews[newIndex].frame = CGRect(
                x: self.backgroundForCircle.subviews[newIndex].frame.origin.x + self.correctionForPoints,
                y: self.correctionForPoints,
                width: self.sizeForCurrentCircle,
                height: self.sizeForCurrentCircle)
